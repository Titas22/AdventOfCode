module AoC_2024_21
    ...
    function get_distance_map(keypad::Keypad)::Dict{Char, Dict{Char, Vector{String}}}
        ...
        for from = CartesianIndices(keypad)
            ...
            for to = CartesianIndices(keypad)
                ...
                # We know each directional input will have to end with an 'A'
                if isa(keypad, DirectionalKeypad)
                    paths .*= 'A'
                end
    ...
    numeric_distances::Dict{Char, Dict{Char, Vector{String}}} = get_distance_map(NUMERIC_KEYPAD)
    directional_distances::Dict{Char, Dict{Char, Vector{String}}} = get_distance_map(DIRECTIONAL_KEYPAD)

    cache::Dict{Tuple{String, Int}, Int} = Dict{Tuple{String, Int}, Int}()
    ...
    function find_input_sequence_length(directional_moves::String; depth::Int)::Int
        ...
        for move in directional_moves
            shortest_move = typemax(Int)
            for path in directional_distances[robot_pos][move]
                if depth == 1
                    path_length =  length(path)
                else
                    path_length =  find_input_sequence_length(path; depth=depth-1)
                end
                path_length < shortest_move || continue
    ...
    function find_shortest_inputs(code::String, num_directional_robots::Int = 2)::Int
        ...
        for num in code
            ...
            for num_path in numeric_paths
                ...
                shortest_num_input > input_length || continue
    ...
    function solve(btest::Bool = false)::Tuple{Any, Any}
        ...
        empty!(cache) # Clean here to not cache between benchmarks
